...
def print_usage():
  print ""
  print "=== WRITE JAVASCRIPT FILES FROM GENOME FILE ==="
  print "Usage: python2 genome_to_js.py [parameters]"
...
### Generate the javascript to render the genome ###
# param PATH : the path of the simulation folder
def generate_genome_js( PATH ):

  genome_filename = PATH+"/statistics/best_genome.txt"
  ...
  for i in range(1, node_index):
    edges.append([i, i+1])
  edges.append([node_index, 1])

  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
  # B) Write JS file                #
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
  f = open("./viewer/src/js/genome_graph.js", "w")
  ...
  f.write("\n")
  f.write("var genome = cytoscape({\n")
  f.write("container: document.getElementById('genome_graph'),\n")
  f.write("\n")
  ...
  for i in range(len(nodes.keys())):
    ...
    #-----------------------------#
    # If the node is BINDING SITE #
    #-----------------------------#
    elif node["ptype"] == BS:
      name = "BS"
      if node["functional"] == 1:
        line += "    { data: { id: '"+str(tag)+"', name: '"+name+"', faveColor: 'rgb(156, 187, 90)', faveShape: 'triangle' } }"
      else:
        line += "    { data: { id: '"+str(tag)+"', name: '"+name+"', faveColor: 'rgb(128, 128, 128)', faveShape: 'triangle' } }"
    #-------------------------#
    # If the node is PROMOTER #
    #-------------------------#
    elif node["ptype"] == P:
      name = "P"
      if node["functional"] == 1:
        line += "    { data: { id: '"+str(tag)+"', name: '"+name+"', faveColor: 'rgb(80, 130, 184)', faveShape: 'star' } }"
      else:
        line += "    { data: { id: '"+str(tag)+"', name: '"+name+"', faveColor: 'rgb(128, 128, 128)', faveShape: 'star' } }"
